...
export default class ListItem extends BaseUi {
    @property({ type: cc.Node, tooltip: CC_DEV && '图标' })
    icon: cc.Node = null;
    ...
    @property({type: cc.Node, tooltip: CC_DEV && '被选标识',visible() { return this.selectedMode > SelectedType.NONE }})
    selectedFlag: cc.Node = null;
    @property({type: cc.SpriteFrame, tooltip: CC_DEV && '被选择的SpriteFrame',visible() { return this.selectedMode == SelectedType.SWITCH }})
    selectedSpriteFrame: cc.SpriteFrame = null;
    //未被选择的SpriteFrame
    _unselectedSpriteFrame: cc.SpriteFrame = null;
    @property({tooltip: CC_DEV && '自适应尺寸（宽或高）',})
    ...
    _registerEvent() {
        if (!this._eventReg) {
            if (this.btnCom && this.list.selectedMode > 0)this.btnCom.clickEvents.unshift(this.createEvt(this, 'onClickThis'));
            if (this.adaptiveSize) this.node.on(cc.Node.EventType.SIZE_CHANGED, this._onSizeChange, this);
            this._eventReg = true;
        }
    }
    ...
    showAni(aniType: number, callFunc: Function, del: boolean) {
        ...
        switch (aniType) {
            case 0: //向上消失
                tween = cc.tween(t.node).to(.2, { scale: .7 }).by(.3, { y: t.node.height * 2 });
                break;
            case 1: //向右消失
                tween = cc.tween(t.node).to(.2, { scale: .7 }).by(.3, { x: t.node.width * 2 });
                break;
            case 2: //向下消失
                tween = cc.tween(t.node).to(.2, { scale: .7 }).by(.3, { y: t.node.height * -2 });
                break;
            case 3: //向左消失
                ...
                break;
            default: //默认：缩小消失
                tween = cc.tween(t.node).to(.3, { scale: .1 });
                break;
        }
        if (callFunc || del) {
            tween.call(() => {
