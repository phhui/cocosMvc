...
@ccclass
    @menu('【部件】/infoBind')
export default class infoBind extends BaseUi {
    ...
    protected _data:{ name?:string,usn?:string,icon?: string, desc?:string, lv:number}=null;
    ...
    public bind(data: { name?:string,usn?:string,icon?: string, desc?:string, lv:number}) {
        this._data=data;
